...
const createInventoryItem = async (req, res) => {
  const { Category, Discription, Brand, Price, Quantity, SellingPrice } = req.body;
  try {
    const newInventoryItem = await Inventory.create({ Category, Discription, Brand, Price, Quantity, SellingPrice });
...
const UpdateinventoryItem = async(req,res)=>{
    ...
    if (!inventoryItem) {
      ...
      res.inventoryItem(200).json(inventoryItem)
}
